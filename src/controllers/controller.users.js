...
const UserControllers = () => {
    ...
    const passwordReset = async (req, res) => {
        ...
        return await GoogleBotChecker(req, captchacode, () => {
            ...
            if (reset.error !== undefined) return res.status(412).json({ error: reset.error })
            return reset
        })
    }
        ...
        return await GoogleBotChecker(req, captcha, () => {
            const validate = loginValidator(email, password, () => {
                pool.query(CHECKEMAILEXISTENCE, [email], (error, found) => {
                    ...
                    if (userObj.status !== 2) return res.status(412).json({ error: IEA })
            ...
            if (validate.error != undefined) return res.status(412).json({ error: validate.error })
            return validate
        })
    }
                    ...
                    if (compareOTP) {
                        ...
                        pool.query(GETUSERBYSLUG, [user], (err, userInfo) => {
                            ...
                            return sign({ ...signedInUser }, process.env.SVCMSMS_JWT_SECRET, { expiresIn: '2h' }, async (tokenerror, token) => {
                                ...
                                res.status(200).json({ message: SL })
                            })
                        })
                    }
                })
            })
            if (confirmLogin.error !== undefined) return res.status(412).json({ error: confirmLogin.error })
            return confirmLogin
        })
    }
